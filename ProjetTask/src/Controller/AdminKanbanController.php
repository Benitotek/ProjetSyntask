...
class AdminKanbanController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        $data = $this->adminKanbanService->getKanbanDataByRole($this->getUser(), $filters);

        // S'assurer que les clés existent pour Twig
    ...
    {
        $this->denyAccessUnlessGranted('ROLE_ADMIN');
        $data = $this->adminKanbanService->getExportData();

        if ($format === 'csv') {
            $fh = fopen('php://temp', 'r+');
            fputcsv($fh, ['ID', 'Titre', 'Statut', 'Priorité', 'Projet', 'Assigné', 'Échéance'], ';');
            foreach ($data['tasks'] as $task) {
                fputcsv($fh, [
                    $task->getId(),
                    method_exists($task, 'getTitle') ? $task->getTitle() : ($task->getTitre() ?? ''),
                    method_exists($task, 'getStatus') ? (is_object($task->getStatus()) && method_exists($task->getStatus(), 'value') ? $task->getStatus()->value : $task->getStatus()) : ($task->getStatut()?->value ?? (string)$task->getStatut()),
                    method_exists($task, 'getPriority') ? (string)$task->getPriority() : (method_exists($task, 'getPriorite') ? (string)$task->getPriorite() : ''),
                    $task->getTaskList()?->getProject()?->getTitre() ?? '',
                    $task->getAssignedUser()?->getUserIdentifier() ?? '',
                    (method_exists($task, 'getDueDate') ? $task->getDueDate() : $task->getDateButoir())?->format('Y-m-d') ?? '',
                ], ';');
            }
            rewind($fh);
            ...
            return new Response($csv, 200, [
                ...
                'Content-Disposition' => 'attachment; filename="kanban.csv"',
            ]);
        }

        if ($format === 'json') {
            return $this->json($data, 200, [
                'Content-Disposition' => 'attachment; filename="kanban.json"'
            ]);
        }
    ...
    {
        $this->denyAccessUnlessGranted('ROLE_ADMIN');
        return $this->json($this->adminKanbanService->getRecentActivitiesForAdmin());
    }
}
