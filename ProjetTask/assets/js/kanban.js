...
/**
 * Kanban JavaScript Module - Version simplifiée et fonctionnelle
 */
...
(function() {
    ...
    function updateColumnCounts() {
        Array.from(document.querySelectorAll('.kanban-column')).forEach(col => {
            const badge = col.querySelector('.badge[data-role="count"]');
            const itemsCount = col.querySelectorAll('.kanban-item').length;
            if (badge) {
    ...
    function initDragAndDrop() {
        if (readOnly || typeof Sortable === 'undefined') {
            console.warn('Drag & Drop désactivé:', readOnly ? 'Mode lecture seule' : 'SortableJS non disponible');
            return;
        }
        ...
        Array.from(document.querySelectorAll('.kanban-list')).forEach(listEl => {
            console.log('Initialisation liste:', listEl);
            ...
            new Sortable(listEl, {
                ...
                onStart: function(evt) {
                    console.log('Début du drag:', evt.item);
                ...
                onEnd: async function(evt) {
                    ...
                    if (oldColumnId !== newColumnId) {
                        try {
                            ...
                            showToast(e.message || 'Erreur lors du déplacement', 'danger');
        ...
        if (board) {
            new Sortable(board, {
                ...
                onEnd: async function() {
                    const columnIds = Array.from(document.querySelectorAll('.kanban-column')).map(el => parseInt(el.dataset.columnId, 10));
                    ...
                    try {
                        ...
                        console.error('Erreur réorganisation colonnes:', e);
                        showToast(e.message || 'Erreur lors de la réorganisation', 'danger');
                    }
                }
            });
        }
    ...
    function initTaskDetails() {
        const modalEl = document.querySelector('#task-details-modal');
        if (!modalEl) return;
        ...
        document.addEventListener('click', async (e) => {
            ...
            try {
                ...
                modalEl.querySelector('.modal-title').textContent = task.titre || task.title || `Tâche #${task.id}`;
                
                const priorityBadge = task.priority || task.priorite ? 
                    `<span class="badge priority-${(task.priority || task.priorite)} me-2">${task.priorityLabel || task.priority || task.priorite}</span>` : '';
                
                const statusBadge = task.statut ? 
                    `<span class="badge statut-${task.statut}">${task.statutLabel || task.statut}</span>` : '';
                
                const description = task.description ? 
                    `<div class="text-muted mb-2">${task.description}</div>` : '';
                
                const dueDate = task.dateButoir || task.dateDeFin ? 
                    `<div class="small"><i class="bi bi-calendar-event me-1"></i> ${task.dateButoir || task.dateDeFin}</div>` : '';
                
                const assignee = task.assignedUser ? 
                    `<div class="small"><i class="bi bi-person me-1"></i> ${task.assignedUser.name || task.assignedUser.fullName || 'Utilisateur'}</div>` :
                    '<div class="small"><i class="bi bi-person me-1"></i> Non assignée</div>';
                
                modalEl.querySelector('.task-details-body').innerHTML = `
                    <div class="mb-2">${priorityBadge}${statusBadge}</div>
                    ${description}
                    ${dueDate}
                    ${assignee}
                `;
                ...
                showToast(e.message || 'Erreur lors du chargement des détails', 'danger');
            }
        });
    }

    // ----------------------
    // Gestion ajout de colonne
    // ----------------------
    function initAddColumn() {
        const btn = document.querySelector('#btn-add-column');
        ...
        btn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                console.error('Erreur création colonne:', e);
                showToast(e.message || 'Erreur lors de la création de la colonne', 'danger');
            }
        });
    }
    ...
    function init() {
        ...
        if (typeof Sortable === 'undefined') {
            ...
            showToast('Erreur: SortableJS requis pour le drag & drop', 'danger');
            return;
        }
...
// const Sortable = window.Sortable || (await import('sortablejs')).default;
// // ----------------------
// // Variables
// // ----------------------
